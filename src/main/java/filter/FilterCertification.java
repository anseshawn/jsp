...
public class FilterCertification implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		// 필터에서는 session 객체 사용 불가, application 객체 사용 가능
		...
		if(!admin.equals("admin")) {
			...
			if(aCertification.equals("")) {
				...
				out.println("history.back()"); // 자바스크립트에서 뒤로 보내는 명령 괄호안에 2를 적으면 두단계 전으로 돌아감	
				out.println("</script>");
				return;
			}
			...
			if(!aUserCertification.equals(aCertification)) {
				if(!u.equals(aCertification)) {
					...
					out.println("<script>");
					out.println("alert('인증 완료 되었습니다. 서비스 이용이 가능합니다.');");
					out.println("</script>");
		...
		if(admin.equals("admin") && aCertification.equals("")) {
			...
			// out.println("location.href='t2_Certification.jsp?admin=admin'"); // 필터 안에서는 location.href 사용불가
			out.println("</script>");			
		}
