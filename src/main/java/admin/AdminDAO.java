...
//	private Connection conn2 = GetConn.getConn(); // 싱글톤 비교해보기 위한 문장
	...
	public ArrayList<ComplaintVO> complaintList() {
		...
		try {
			sql = "select date_format(c.cpDate,'%Y-%m-%d %H:%i') as cpDate, c.*,"
					+"b.title,b.nickName,b.mid,b.complaint as complaint from complaint c, board b "
					+"where c.partIdx = b.idx order by idx desc";
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
	...
	// 리뷰 내역 전체리스트 가져오기
	public ArrayList<ReviewVO> getReviewList(int partIdx, String part) {
		ArrayList<ReviewVO> rVos = new ArrayList<ReviewVO>();
		try {
			// sql="select * from review where part = ? and partIdx = ? order by idx desc";
			sql="select * from (select * from review where part=? and partIdx=?) as v"
					+ " left join reviewReply r on v.partIdx=? and v.idx=r.reviewIdx"
					+ " order by v.idx desc, r.replyIdx desc";
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, part);
			pstmt.setInt(2, partIdx);
			pstmt.setInt(3, partIdx);
			rs = pstmt.executeQuery();
			while(rs.next()) {
