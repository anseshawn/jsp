...
%>
		...
		su3 + su4 = ${(su3+su4)}<br/>
		su3 * su4 = ${(su3*su4)}<br/>
		<!-- EL 표기법에서는 수치로 바꾸어 연산 -->
	</div>
	<hr/>
	<h4>파라미터(parameter)를 통해서 넘어온 값의 처리(QueryString형식)</h4>
	<div>Form태그의 get/post 방식의 전송이나, url을 통한 값의 전송(주소?변수1=값1&변수2=값2...) 되는 값의 처리</div>
	<div>주소창에 현재주소 뒤에 '?mbc=200&kbs=50'을 입력 후 아래 내용 확인하시오.<br/>
		mbc = ${param.mbc}<br/>
		kbs = ${param.kbs}<br/>
...
<%
	...
	// if문에서 EL을 비교할 수 없기 때문에 name을 다시 불러와야 함
	if(!name.equals("")) {
%>
		<p>성명 : ${name}</p>
		<p>취미 : ${hobby}</p>
