...
<div class="container">
	...
	<pre>
		사용법1 : < c : forEach var="변수" begin="초기값" end="최종값" step="증감값" varStatus="매개변수(변수의 상태)" > < / c : forEach >
					: step이 1인 경우는 생략 가능하다. varStatus="변수" 는 필요할 때만 선언해서 사용하면 된다.
...
%>
	...
	<div>
		<c:set var="images" value="111/112/113/114/115" />
		<c:forTokens var="img" items="${images}" delims="/" varStatus="st">
			<img src="${ctp}/images/${img}.jpg" width="100px" />
			<c:if test="${st.count % 3 == 0}"><br/></c:if>
		</c:forTokens>
	</div>
	<br/><br/>
	
	<h5>5.2차원 형식의 배열에 저장된 자료를 한 행에 3개의 항목씩 출력하시오.</h5>
<%
	String[][] members = {
			{"홍길동","23","서울"},	
			{"김말숙","26","청주"},	
...
%>
	...
	<h5>8. 앞의 사용법2번에 저장한 cards의 내용 중, 첫번째 카드의 배경색은 노란색, 마지막 카드의 배경색은 하늘색</h5>
	<c:forEach var="card" items="${cards}" varStatus="st">
