...
<div class="container">
	...
	<div class="text-right">
		<button type="button" onclick="location.href='jstl1.jsp';" class="btn btn-warning btn-sm">JSTL core라이브러리</button>
	...
	<pre>
		사용용도 : 형식문자열을 표현(지정)할 때 사용한다.(예: 쉼표, 통화량(화폐)단위, 백분율..., 날짜형식...)
		사용법 : < fmt : 명령어 ... value="$ {값 / 변수}" pattern="표현할 형식(패턴)" [type="화폐단위"] />
	</pre>
	...
	<div>
		...
		<fmt:formatNumber value="${won1}" pattern="0,000"/> : <fmt:formatNumber value="${won2}" pattern="0,000"/> | <fmt:formatNumber value="7654.521" pattern="0,000"/> <br/>
		<fmt:formatNumber value="${won1}" pattern="0,000.0"/> : <fmt:formatNumber value="${won2}" pattern="0,000.0"/> | <fmt:formatNumber value="7654.561" pattern="0,000.0"/> <br/>
		<fmt:formatNumber value="${won1}" pattern="#,##0.0"/> : <fmt:formatNumber value="${won2}" pattern="#,##0.0"/> | <fmt:formatNumber value="4.561" pattern="#,##0.0"/> <br/>
		<!-- 0은 자릿수 확보, 서식기호 '#'은 의미가 없는 0을 표시하지 않는다. -->
		<hr/>
		
		2. 화폐단위<br/>
		원화 : <fmt:formatNumber value="${won1}" type="currency"/><br/>
		USD1 : <fmt:formatNumber value="${won1}" type="currency" currencyCode="USD"/><br/>
		USD2 : <fmt:formatNumber value="${won2}" type="currency" pattern="#,##0.00" currencyCode="USD"/><br/>
		<hr/>
		...
		현재시간3 : <fmt:formatDate value="${today}" pattern="HH : mm : ss"/><br/>
		...
		톰캣서버의 기본 로케일을 미국식으로 변경 : <fmt:setLocale value="en_US"/>
					...
					<!-- 미국으로 변경되었기 때문에 미국 통화량으로 출력됨 -->
		<hr/>
		
		6. URL 이동 : location.href = ''; ==>>> redirect : core라이브러리에 있음<br/>
								(예1: < c : redirect url = '경로' / >)
								(예2: < c : redirect url = '경로' >
												< c : param name = "변수명" value="값" / >
										 < / c : redirect > )
		
		7. import : core 라이브러리에 있음
