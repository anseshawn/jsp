	...
	<table class="table table-bordered">
		...
		<tr>
			<td colspan="4">
				<div class="row">
	      	<div class="col">
						<input type="button" value="돌아가기" onclick="location.href='BoardList.ad?pag=${pag}&pageSize=${pageSize}';" class="btn btn-secondary"/>
					</div>
				</div>
			</td>
		</tr>
		...
		<tr>
			<td>
				<c:if test="${!empty nextVo.title}">
					<i class="fa-solid fa-angles-up"></i><a href="BoardContent.ad?idx=${nextVo.idx}"> 다음글 : ${nextVo.title}</a><br/>
				</c:if>
				<c:if test="${!empty preVo.title}">
					<i class="fa-solid fa-angles-down"></i><a href="BoardContent.ad?idx=${preVo.idx}"> 이전글 : ${preVo.title}</a><br/>
				</c:if>
			</td>
		</tr>
	</table>
