  	...
  	function hInput() {
  		...
  		$.ajax({
  			...
  			success: function(res) {
  				if(res == "1") {
  					alert("회원 등록 완료");
  					// ajax로 처리하면 화면 안에 머물러 있게 되기 때문에 새로고침을 따로 해줘야 함
  					location.reload();
  				}
  				else {
  					alert(res);
  	...
  	function hUpdate() {
  		...
  		$.ajax({
  			...
  			success: function(res) {
  				if(res != "0") {
  					alert("회원 정보 수정 완료");
  					// location.reload(); // 전체 화면 reload
  					
  					// 부분 reload()
  					$("#hoewonList").load(location.href = ' #hoewonList');
  				}
  				else {
  					alert("수정 실패");
  					$("#pwd").focus();
		...
		<table class="table table-bordered">
			...
			<tr>
				<td colspan="2" class="text-center">
					<input type="button" onclick="hInput()" value="회원등록" class="btn btn-success mr-2"/>
					<input type="reset" value="다시입력" class="btn btn-warning mr-2"/>
					<input type="button" onclick="hUpdate()" value="회원수정" class="btn btn-secondary mr-2"/>
					<input type="button" onclick="location.href='hoewonList.st';" value="전체보기" class="btn btn-primary mr-2"/>
				</td>
			</tr>
		</table>
	</form>
		...
		<table class="table table-hover text-center">
			...
			<c:forEach var="vo" items="${vos}" varStatus="st">
				<tr>
					...
					<td>${vo.address}</td>
					<td>
						<a href="javascript:hoewonSearch(${vo.idx})" class="btn btn-primary btn-sm">개별조회</a>
